...
function getLifeCycleTarget(target) {
  return target || (0, lib_exports.getCurrentInstance)();
}
function tryOnUnmounted(fn, target) {
  const instance = getLifeCycleTarget(target);
  if (instance)
    (0, lib_exports.onUnmounted)(fn, target);
}
...
function useEventListener(...args) {
  ...
  const stopWatch = (0, lib_exports.watch)(
...
function useMounted() {
  const isMounted = (0, lib_exports.ref)(false);
...
function useMediaQuery(query, options = {}) {
  ...
  let mediaQuery;
  const matches = (0, lib_exports.ref)(false);
  const handler = (event) => {
  ...
  const stopWatch = (0, lib_exports.watchEffect)(() => {
...
function useIntersectionObserver(target, callback, options = {}) {
  ...
  const targets = (0, lib_exports.computed)(() => {
    const _target = toValue(target);
    return (Array.isArray(_target) ? _target : [_target]).map(unrefElement).filter(notNullish);
  });
  let cleanup = noop;
  const isActive = (0, lib_exports.ref)(immediate);
